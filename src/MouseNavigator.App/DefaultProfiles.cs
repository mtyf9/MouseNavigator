...
internal static class DefaultProfiles
{
    ...
    [
        new(1, "global", "全局导航", [],
        [
            new(RingSlot.Top, "windows.tasks"), new(RingSlot.Right, "windows.window.next"),
            new(RingSlot.Bottom, "windows.minimizeAll"), new(RingSlot.Left, "windows.window.previous")
        ]),
        // First-run example; saved user menus take precedence on subsequent launches.
        new(1, "explorer", "文件资源管理器", [new("explorer.exe")],
        [
            new(RingSlot.Top, "windows.maximize"), new(RingSlot.Right, "windows.window.next"),
            new(RingSlot.Bottom, "windows.minimizeAll"), new(RingSlot.Left, "windows.window.previous")
        ], 10)
    ];
}
